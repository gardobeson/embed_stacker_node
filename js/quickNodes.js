...
app.registerExtension({
  name: "gardobeson.ESN.QuickNodes",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (
      nodeData.name === "embed_stacker_node" ||
      nodeData.name === "CLIPTextEncode"
    ) {
      addMenuHandler(nodeType, function (_, options) {
        options.unshift({
          content: `Prepend Embed Stacker`,
          callback: () => {
            const newNode = placeNewNode("embed_stacker_node", this, {
              before: true,
            ...
            try {
              ...
              if (prompts && typeof prompts !== "undefined") {
                newNode.widgets[6].value = prompts;
              }
            } catch (e) {
